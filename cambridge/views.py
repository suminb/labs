...
from cambridge import translate as cambridge_translate

def index(request, id='0'):
    r = request.REQUEST
    
    # I probably should've made a custom template tag for this, but I was too lazy.
    description = cambridge_translate(u'물론 캠브릿지 대학에서 실제로 이런 연구가 진행되었다는 사실이 확인된 적은 없습니다. 다만 이 문구가 인터넷을 통해 퍼지기 시작했기 때문에 앱 이름을 캠브릿지라고 지은것 뿐입니다.')
...
def translate(request):
    if 'q' in request.GET:
        ...
        return response_ok({'result': cambridge_translate(query)})
